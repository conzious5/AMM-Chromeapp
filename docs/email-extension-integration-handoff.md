...
Tests and deployment: commit `09834ec` is on `main`; typecheck and build pass; Vitest passes 5 files / 27 tests. Regression coverage includes the payment draft versus older apparel/pricing/shipping history, regeneration, fallback, source segmentation, empty draft behavior, mode separation, and privacy-safe topic-drift error classification. Railway deployment `8f872538-33ac-4d85-a7b7-88af0339c0ae` is active and health returned 200/ok. Six synthetic production cases passed: payment/apparel drift, delivery, client excitement, multiple questions, changed-subject/old-pricing, and Zac's Edit. All returned HTTP 200 with required intent signals present and forbidden facts/topics absent; no rewrite failure or `invalid_json_schema` appeared, and the temporary test session was revoked.
