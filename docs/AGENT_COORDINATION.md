...
| `feature/extension-experience` | Chrome extension UI, Gmail compose experience, local mocks/tests | Implemented through context-grounding companion `b88bce5`, with canonical native-auth adapter at `2ebc687`; canonical server auth, Prisma, portal, analytics, Railway, and Meeting Coach are untouched. |
...
- AMM Style intent-preservation follow-up: commit `09834ec` is on `main` and active in Railway deployment `8f872538-33ac-4d85-a7b7-88af0339c0ae`. Backend source segmentation, topic/provenance guard, one constrained regeneration, fail-safe draft fallback, and apparel/payment regression coverage are complete. Six privacy-safe production scenarios passed with HTTP 200 and no forbidden-topic leakage. Agent 3's separate context-cleanup and warning-mapping companion is committed at `b88bce5` on `feature/extension-experience`. No auth, sender-permission, Prisma, portal, or Meeting Coach changes are included.
