...
| `feature/extension-experience` | Chrome extension UI, Gmail compose experience, beta package/validation, new-outbound-email coaching capture | Extension-owned implementation through `06c039d`; canonical server auth, Prisma, portal, analytics, Railway, Meeting Coach, and coaching backend remain unchanged from `main`. |
...
- Live installed-beta tracing on September 16, 2026 confirmed the local session problem is past the previous failure boundary: an expired access token received one successful refresh, the retried `GET /api/extension/config` returned 200, and `POST /api/rewrite` reached production. The rewrite returned 502 only after OpenAI rejected the generated structured-output schema with `invalid_json_schema` because installed `openai@5.23.2` + `zod@4.6.5` converted the object-shaped `rewriteModelResultSchema` into JSON Schema `{ "type": "string" }`. **Integration dependency:** canonical OpenAI service ownership must correct and test the schema adapter; the extension branch does not independently change that shared service. Extension-only pending work adds service-worker-authoritative session restoration, stale-refresh guards, privacy-safe auth diagnostics, and specific error categories without releasing a new beta.
- Installed v0.1.2 testing confirmed a single compose-compatibility defect: controls appeared in standalone New Compose but not Reply, Reply All, or Forward. Commit `06c039d` generalizes compose discovery from dialog-only roots to Gmail's live `g_editable` body plus semantic dialog/region boundary, preserves one shared adapter/lifecycle, distinguishes current Reply/Reply All/Forward markers, falls back to protected config's authenticated user only when Gmail omits default From metadata, and advances the manifest to 0.1.3. Automated extension coverage passes 49/49. Live Chrome DOM inspection confirmed all four mode structures and safe anchors, but the branch build could not be loaded through blocked `chrome://extensions` automation; installed-build retesting remains required. No v0.1.3 release exists yet, and release remains gated by the canonical OpenAI schema fix.

Authoritative continuation handoff: `docs/email-extension-integration-handoff.md` on `feature/extension-experience`; implementation commits `657c95f`, `6eb8e9e`, `2ebc687`, `afc8f45`, beta-validation merge `1b75312`, v0.1.1 fetch hotfix `34bb946`, v0.1.2 lifecycle fix `5132cb1`, outbound-coaching capture `2d16e1b`, auth-state hardening `edd37d0`, and inline-compose compatibility `06c039d`; initial handoff commit `ae97860`.
...
- Production rewriting depends on canonical OpenAI service repair of the Zod 4 / SDK structured-output conversion described above. The backend currently collapses the upstream `invalid_json_schema` HTTP 400 into a generic rewrite HTTP 502; extension-side mapping can identify the model stage from that contract, but cannot make rewriting succeed.
