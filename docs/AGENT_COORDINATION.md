...
| `feature/extension-experience` | Chrome extension UI, Gmail compose experience, beta package/validation, new-outbound-email coaching capture | Extension-owned v0.1.3 release implementation through `d91aaaf`; canonical server auth, Prisma, portal, analytics, Railway, Meeting Coach, and coaching backend remain unchanged from `main`. |
...
- Live installed-beta tracing on September 16, 2026 confirmed the local session problem was past the previous failure boundary. Canonical `main` commits `89ea1a5` and `166998e` subsequently repaired rewrite and training structured-output schemas; production verification recorded in `f19f221` confirms AMM Style, Zac's Edit, and training analysis now return valid structured output. The former `invalid_json_schema` release blocker is resolved.
- Installed v0.1.2 testing confirmed a single compose-compatibility defect: controls appeared in standalone New Compose but not Reply, Reply All, or Forward. Commit `06c039d` generalizes compose discovery from dialog-only roots to Gmail's live `g_editable` body plus semantic dialog/region boundary, preserves one shared adapter/lifecycle, distinguishes current Reply/Reply All/Forward markers, falls back to protected config's authenticated user only when Gmail omits default From metadata, and advances the manifest to 0.1.3. Automated extension coverage passes 49/49. Live Chrome DOM inspection confirmed all four mode structures and safe anchors; installed v0.1.3 manual retesting remains required.
- `v0.1.3-beta` is published from `feature/extension-experience` release commit `d91aaaf`: `https://github.com/conzious5/AMM-Chromeapp/releases/tag/v0.1.3-beta`. Direct asset: `https://github.com/conzious5/AMM-Chromeapp/releases/download/v0.1.3-beta/AMM-Voice-Beta-v0.1.3.zip`. SHA-256 `330411522f07f72d21223d9645a6daba52675ee401fbf1e2e0b647d7cf6cf195`; the downloaded GitHub asset matched the validated local artifact byte-for-byte. Pre-publish validation: 49/49 extension tests, all runtime JavaScript syntax checks, Manifest V3/version/reference/permission checks, ZIP integrity, explicit 15-file allowlist, exact source-byte comparison, and zero secret-scan findings. Manual installed-Chrome testing remains required across all four compose modes and both authorized senders.
- Post-v0.1.3 extension context-quality companion work preserves the canonical rewrite schema while treating `draft` as Current draft and labeling selected `thread` content as Latest inbound, Recent relevant thread, and Older history. It reduces rewrite context to five messages/12,000 characters, uses backend-authorized sender identities only in memory to avoid guessing inbound direction, filters unrelated older history, and maps explicit `TOPIC_DRIFT`, `UNSUPPORTED_FACT`, and `UNRELATED_THREAD_CONTEXT` warnings to a careful-review message. The separate deferred outbound-coaching context boundary remains unchanged. No release has been cut for this work.

Authoritative continuation handoff: `docs/email-extension-integration-handoff.md` on `feature/extension-experience`; implementation commits `657c95f`, `6eb8e9e`, `2ebc687`, `afc8f45`, beta-validation merge `1b75312`, v0.1.1 fetch hotfix `34bb946`, v0.1.2 lifecycle fix `5132cb1`, outbound-coaching capture `2d16e1b`, auth-state hardening `edd37d0`, inline-compose compatibility `06c039d`, and v0.1.3 install/release preparation `d91aaaf`; initial handoff commit `ae97860`.
...
- Production rewriting structured-output dependency is resolved on canonical `main` by `89ea1a5` and `166998e`; production AMM Style, Zac's Edit, and training analysis have returned valid structured output.
