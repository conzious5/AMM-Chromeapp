...
const testConfig: AppConfig = {
  ...
  PUBLIC_BASE_URL: "http://localhost:3000", SESSION_SECRET: "development-session-secret-change-me", ADMIN_EMAILS: "", TEAM_EMAILS: "", EXTENSION_IDS: "", USER_SENDER_PERMISSIONS_JSON: "{}"
};
...
describe("API", () => {
  it("has a public health endpoint", async () => {
    const app = await buildApp({ config: testConfig, auth: new DevelopmentTokenAuth(testConfig.DEV_AUTH_TOKEN), rewriteService: new RewriteService(repository, model), analytics: new AnalyticsRepository() });
