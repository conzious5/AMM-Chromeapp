...
async function settings() {
  const value = await chrome.storage.local.get(["backendUrl", "authToken", "developmentMode", "mockAuthorizedSenders"]);
  return {
...
async function services() {
  ...
  const auth = developmentAllowed
    ...
    : new AMMVoiceAuth.BackendExtensionAuthProvider(chrome, settings);
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  (async () => {
    ...
    if (message.type === "SIGN_IN") { await auth.signIn(); return client.getCurrentUser(); }
