(function (root, factory) {
  ...
  class ExtensionApiClient {
    ...
    async request(path, init = {}) { const token = await this.auth.getAccessToken(); if (!token) throw new Error("SIGN_IN_REQUIRED"); const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), this.timeoutMs); try { const response = await this.fetch(`${this.backendUrl}${path}`, { ...init, signal: controller.signal, headers: { "content-type": "application/json", authorization: `Bearer ${token}`, ...(init.headers || {}) } }); if (response.status === 401) { await this.auth.signOut(); throw new Error("AUTHENTICATION_EXPIRED"); } const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || `API failure (${response.status})`); return body; } finally { clearTimeout(timeout); } }
