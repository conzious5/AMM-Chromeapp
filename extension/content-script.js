(function () {
  ...
  function renderResult(state, output) {
    ...
    const reviewItems = state.mode === "zacs_edit" && state.settings.showZacReview ? (output.reviewNotes || []).filter(Boolean) : []; listItems(ui.reviewList, reviewItems); ui.review.hidden = reviewItems.length === 0;
