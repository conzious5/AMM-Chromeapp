...
async function load() {
  ...
  fields.environment.value = value.backendEnvironment || "production"; fields.backend.value = value.backendUrl || (fields.environment.value === "local" ? LOCAL_BACKEND : DEFAULT_BACKEND); fields.defaultAction.value = value.defaultAction || "amm_style"; fields.showReview.checked = value.showZacReview !== false; fields.autoSender.checked = value.autoDetectSender !== false; fields.developmentMode.checked = Boolean(value.developmentMode); environmentChanged(); try { showCoachingPolicy(await send({ type: "GET_CONFIG" })); } catch { /* Sign-in is optional while viewing settings. */ }
}
async function save() {
...
document.querySelector("#signin").addEventListener("click", async () => { let password = ""; try { await save(); fields.status.textContent = "Signing in…"; password = fields.password.value; fields.password.value = ""; const config = await send({ type: "SIGN_IN", email: fields.email.value, password }); showCoachingPolicy(config); fields.status.textContent = `Signed in as ${config.authenticatedUser}. Authorized From addresses: ${(config.senderAddresses || []).join(", ") || "none"}.`; } catch (error) { fields.status.textContent = error.message; } finally { password = ""; fields.password.value = ""; } });
document.querySelector("#signout").addEventListener("click", async () => { try { await send({ type: "SIGN_OUT" }); fields.coachingPolicy.textContent = "Sign in to view the current coaching policy."; fields.status.textContent = "Signed out."; } catch (error) { fields.status.textContent = error.message; } });
